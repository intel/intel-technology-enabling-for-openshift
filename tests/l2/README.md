...
#### GPU testcase
The testcase used is [clinfo](https://github.com/Oblomov/clinfo), which displays the related information of GPU card. The OCP buildconfig is leveraged to build clinfo container image and push it to the embedded repository through OCP imagestream.
The Job pod is scheduled on a node with GPU card and the resource ```gpu.intel.com/i915``` is registered by the GPU device plugin.
...
A sample result for clinfo detecting GPU card:
```
...
#### SGX test case
The test case used is SGX SDK [Sample Enclave App](https://github.com/intel/linux-sgx/tree/master/SampleCode/SampleEnclave), which launches a simple SGX enclave. Similar to GPU,  OCP buildconfig and imagestream are leveraged for the container image.
